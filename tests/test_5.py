...
if __name__ == "__main__":
    print("--- Configuración de Simulación de Gráfica ---")
    
    try:
        # Solicitamos los valores al usuario
        n_nodos = int(input("Introduce el número de nodos (ej. 10): "))
        p = float(input("Introduce la probabilidad de arista (0 a 1, ej. 0.3): "))

        # Validamos que la probabilidad sea correcta
        if not (0 <= p <= 1):
        ...
        print("Error: Por favor introduce números válidos (enteros para nodos, decimales para probabilidad).")
